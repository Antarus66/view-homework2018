...
2. Страница отдельной валюты
    ...
    - Страница должна содержать список с лотами к этой валюте. Будут показаны цены.
    - Каждый лот должен быть кликабелен и вести к странице лота.
        - Название валюты
        - Цена
        - Игрок который продает валюту и его имя и номер телефона.
...
- Приветствуется (но не обязательно) использование Material, Bootstrap или другого front-end фреймворка для добавления стилей.
- Возможности Vue.JS будут проверены в последующих домашках. Если заюзаете, будет супер.
- Очень приветствуется добавление возможностей по редактированию, добавлению или удалению лотов валют.
